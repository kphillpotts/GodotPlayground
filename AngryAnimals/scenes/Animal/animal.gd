...
func _ready():
	_start = global_position # Replace with function body.
...
func update_debug_label() -> void:
	var s = "g_pos: %s\n contacts: %s" % [Utils.vect2_to_str(global_position), get_contact_count()]
	s+= "dragging: %s - released: %s\n" % [_dragging, _released]
	s+= "start: %s drag_start: %s drag_vector: %s\n" % [Utils.vect2_to_str(_start), Utils.vect2_to_str(_drag_start), Utils.vect2_to_str(_dragged_vector)]
	s+= "last_drag_position: %s last_drag_amount: %.1f \n" % [Utils.vect2_to_str(_last_dragged_position), _last_drag_amount]
	s+= "ang: %.1f linear: %s fired_time %.1f" % [angular_velocity, Utils.vect2_to_str(linear_velocity), _fired_time]
...
func play_collision() -> void:
	if (_last_collision_count == 0 and get_contact_count() > 0 and collision_sound.playing == false):
